...
def add():
    ...
    if title:
        mutate_todos(lambda todos: todos.append({"id": next_id(todos), "title": title, "done": False, "priority": priority, "due_date": due_date}))
...
def api_add():
    ...
    result = {}
    def _api_add(todos):
        todo = {"id": next_id(todos), "title": title, "done": False, "priority": priority}
        todos.append(todo)
        result.update(todo)
